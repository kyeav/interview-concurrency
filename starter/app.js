...
// ------------------
async function main() {
  console.log(await Promise.all([fetchUserData(0), fetchUserData(1)]));
  // empty array of length 100, fetch user ids
  const userIds = await fetchFollowerIds();
  // array of 100 Promises, map user ids to promises
  const users = userIds.map((id) => fetchUserData(id));
  // array of 100 user objects, wait for user data inside promises
  const usersData = await Promise.all(users);
  // inside followers div, turn user data into html
